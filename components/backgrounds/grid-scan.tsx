...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  float lineMask = max(primaryMask, altMask);
	float fadeStrength2 = fadeStrength * (1.0 - smoothstep(0.0, 1.5, dist));
    
	float fade = exp(-dist * fadeStrength);
    if (uIsLightTheme > 0.5) {
    	fade = exp(-dist * fadeStrength) + 0.15;
  ...
  if (uIsLightTheme < 0.5) {
    vec3 gridColDark = uLinesColor * lineMask * fade;
    color = gridColDark + scanCol + scanAura;
    float n2 = fract(sin(dot(gl_FragCoord.xy + vec2(iTime * 123.4), vec2(12.9898,78.233))) * 43758.5453123);
    color += (n2 - 0.5) * uNoise;
    color = clamp(color, 0.0, 1.0);
    alpha = clamp(max(lineMask, combinedPulse), 0.0, 1.0);
    alpha = max(alpha, halo * clamp(uBloomOpacity, 0.0, 1.0));
  }
	...
	useEffect(() => {
		...
		const onClick = async () => {
			...
			if (
				...
				const requestPermission = devOrientation.requestPermission;
		...
		>
			{showPreview && (
				<div className='absolute right-3 bottom-3 w-55 h-33 rounded-lg overflow-hidden border border-white/25 shadow-[0_4px_16px_rgba(0,0,0,0.4)] bg-black text-white text-[12px] leading-[1.2] font-sans pointer-events-none'>
					...
					/>
					<div className='absolute left-2 top-2 px-1.5 py-0.5 bg-black/50 rounded-[6px] backdrop-blur-xs'>
						{enableWebcam
							? modelsReady
								? uiFaceActive
									? 'Face: tracking'
